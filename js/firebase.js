 ...
 * @param {string} employeeId
 */
async function removeFridagShiftsFromFirebase(employeeId) {
  ...
  for (const doc of schedulesSnapshot.docs) {
    ...
    const filteredShifts = shifts.filter(s => {
      // Keep if not this employee
      if (s.employeeId !== employeeId) return true;

      // Keep if not FP/FPV
      if (s.badge !== 'fp' && s.badge !== 'fpv') return true;

      // Remove if FP/FPV with time '-' (fridagsnyckel-generated)
      if (s.time === '-') {
        removedCount++;
        return false;
      }

      // Keep FP/FPV with actual time (from PDF upload)
      return true;
    });

    // Update if any shifts were removed
    if (filteredShifts.length !== shifts.length) {
      if (filteredShifts.length > 0) {
        await db.collection('schedules').doc(doc.id).set({ shifts: filteredShifts });
      } else {
        // Delete the document if no shifts remain
        await db.collection('schedules').doc(doc.id).delete();
      }

      // Also update local data
      if (filteredShifts.length > 0) {
        employeesData[doc.id] = filteredShifts;
      } else {
        delete employeesData[doc.id];
      }
    }
  }
